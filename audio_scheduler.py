...
# audio_scheduler.py
#
# Periodically processes audio files for contact detail extraction, archives processed files,
# and triggers Excel export of results. Designed for use in automated or batch workflows.
# =============================================================================
...
def process_audio_files(config):
    ...
    while True:
        ...
        for audio_file in audio_files:
            ...
            try:
                # 1. Transcribe audio and extract entities from the audio file
                ...
                # 2. Save extracted details as a JSON file in the output directory
                output_filename = os.path.splitext(audio_file)[0] + ".json"
                output_path = os.path.join(output_dir, output_filename)
                with open(output_path, "w", encoding="utf-8") as f:
                    json.dump(output_data, f, ensure_ascii=False, indent=2)
                # 3. Move the processed audio file to the archive directory (overwrite if exists)
        ...
        # 4. After processing all files in this poll, trigger Excel export of all JSON results
        print("Scheduler finished, triggering Excel export...")
        export_excel()
...
if __name__ == "__main__":
    # Load config from external JSON file and start the scheduler
